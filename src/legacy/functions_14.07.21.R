...
Save_prot_chunk <- function(proteome, Nmers=Nmers, nF=nF, MiSl=MiSl, orderedProteomeEntries=orderedProteomeEntries, directory=directory){
  for(j in 1:nF){
    start=Pi[j]+1
    end=Pi[j+1]
    
    inputSequence = proteome[orderedProteomeEntries[start:end]]
    # save(inputSequence, file=paste(directory, "/inputSequence_", start, "_", end, ".RData" ,sep=''))
    seqinr::write.fasta(sequences = inputSequence, names = names(inputSequence), 
                        file.out = paste0(directory, "/inputSequence_", start, "_", end, ".fasta"))
  }
}
...
computeSPcomplete <- function(cp,maxL,minL,MiSl){
  ...
  return(SP)
  rm(temp, temp1, temp2, temp3, temp4, N, NN, L, ind, SP, a)
}
