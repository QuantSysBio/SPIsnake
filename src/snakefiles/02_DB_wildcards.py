...
# Checkpoint to specify combinations of proteomes and chunks
class Checkpoint_MakePattern:
    ...
    def get_proteomes(self):
        #pd.read_csv(features["Master_table"], sep=",")
        proteomes = Master_table["Proteome"]
        return proteomes

    def get_chunks(self):
        proteomes = self.get_proteomes()
        chunks, = glob_wildcards(
                join(dir_DB_exhaustive, "../{chunk}.fasta"), 
            )
        return chunks
    ...
    def __call__(self, w):
        ...
        # wait for the results of 'check_Split_proteomes'; 
        #this will trigger an exception until that rule has been run.
        checkpoints.check_Split_proteomes.get(**w)

        # expand the pattern based on inputs
        proteomes = self.get_proteomes()
        chunks = self.get_chunks()

        names_out = expand(self.pattern, proteome=proteomes, chunk=chunks, **w)
        return names_out
#chunks, = glob_wildcards(join(dir_DB_exhaustive, "{proteome}/{proteome}_{chunk}.fasta"))
